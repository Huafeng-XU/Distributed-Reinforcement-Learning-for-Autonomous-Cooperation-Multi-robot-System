...
def run(config):
    log_dir = 'checkpoints_ddpgFP_no_rnn_1_24/'
    run_dir = log_dir + 'logs/'
    logger = SummaryWriter(str(log_dir))

    env = Env()
    ddpgFP = DDPGFP.init_from_env(agent_alg=config.agent_alg,use_rnn=False,
                                  tau=config.tau,
                                  lr=config.lr,
                                  hidden_dim=config.hidden_dim)
    replay_buffer = ReplayBuffer(config.buffer_length, ddpgFP.nagents,
                                 [362]*3,
                                 [4]*3)
    t = 0
    for ep_i in range(0, config.n_episodes):
        print("Episodes %i-%i of %i" % (ep_i + 1,
                                        ...
                                        config.n_episodes))
        obs = env.reset()
        ddpgFP.prep_rollouts(device='cpu')
        ...
        for et_i in range(config.episode_length):
            ...
            torch_agent_actions = ddpgFP.stepAll(torch_obs, explore=True)
